...
def store_df(data_type,res,run):
    #       NNval NNTest
    #  params.seed
    # method_list = ['weighted ROC OVO','weighted AUPRC','macro ROC OVR','weighted ROC OVR','macro ROC OVO','micro AUPRC', 'samples AUPRC',  'macro AUPRC']
    ...
    if not os.path.isfile('compare_AUC.csv'):
        types = ['val','test']
        col_list =['RUNS']
        ...
        df.to_csv('compare_AUC.csv',index=False)
    # import pdb;pdb.set_trace()

    # import pdb;pdb.set_trace()
    df_full = pd.read_csv('compare_AUC.csv',index_col='RUNS')
    ...
    df_full.to_csv('compare_AUC.csv',index=True)
...
for al_id,alpha in enumerate(alpha_t):
    ...
    print(confusion_matrix(lbls, preds),"\n",classification_report(lbls, preds, target_names=['cor']+[str(i) for i in range(1,n_class)],labels=[i for i in range(n_class)]))
