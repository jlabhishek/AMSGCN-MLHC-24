...
class Processor():
    ...
    def load_data(self):
        ...
        if self.arg.phase == 'train':
            ...
            self.data_loader[self.arg.phase] = torch.utils.data.DataLoader(
                dataset=Feeder(**self.arg.test_feeder_args),
                batch_size=self.arg.test_batch_size,
                shuffle=False,
                num_workers=self.arg.num_worker,
    ...
    def load_model(self):
        ...
        if self.arg.weights:
            self.global_step =0# int(arg.weights[:-3].split('-')[-1])
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            self.global_step += 1
            # get data
            data = Variable(data.float().to(self.output_device), requires_grad=False)
            label = Variable(label.long().to(self.output_device), requires_grad=False)
    ...
    def eval(self, epoch, save_score=False, loader_name=['test'], wrong_file=None, result_file=None):
        ...
        for ln in loader_name:
            ...
            self.es(self.model,loss)
               
            print(f"Epoch: {epoch}, vloss: {loss:>7f}, EStop:[{self.es.status}]")
        ...
        if self.arg.phase == 'train':
            ...
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                ...
                self.eval(
                    ...
                    loader_name=['val'])
            self.print_log(' \tbest accuracy: {:.4f}.,es best loss:{:.4f}., stopped at {:d} epoch'.format(self.best_acc,self.es.best_loss,self.arg.num_epoch-self.es.counter))

            print('best accuracy: ', self.best_acc, ' model_name: ', self.arg.model_saved_name, ' es best loss',self.es.best_loss)
            state_dict = self.es.best_model.state_dict()
            weights = OrderedDict([[k.split('module.')[-1],
                                    v.cpu()] for k, v in state_dict.items()])

            torch.save(weights, self.arg.model_saved_name + '-best.pt')

            state_dict = self.model.state_dict()
            weights = OrderedDict([[k.split('module.')[-1],
                                    v.cpu()] for k, v in state_dict.items()])

            torch.save(weights, self.arg.model_saved_name + '-last.pt')

        elif self.arg.phase == 'test' or self.arg.phase == 'val':
            ...
            self.eval(epoch=0, save_score=self.arg.save_score, loader_name=[self.arg.phase], wrong_file=wf, result_file=rf)
            self.print_log('best accuracy: {} Done.\n'.format(self.best_acc))
